...
test('operator routes share a concise Russian menu and exact release stamp', async () => {
  await withServer(async (url) => {
    ...
    for (const path of pages) {
      ...
      assert.match(page, /Версия 3\.4\.0 · релиз 15\.09\.2026, 21:40:00 UTC/u);
    ...
    assert.match(help, /Последние пять сохранённых вопросов/u);
    ...
    assert.match(help, /Очередь ручной проверки в «Модерации» включается только отдельной утверждённой настройкой/u);
    ...
    assert.deepEqual(await (await fetch(url + '/api/operator/release', {
      headers: { authorization: auth() },
    })).json(), { version: '3.4.0', releasedAt: '2026-09-15T21:40:00Z' });
