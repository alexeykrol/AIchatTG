...
test('malformed JSON, wrong result kind and invalid analysis cannot prove router no-call', (t) => {
  const { db, config } = fixture(t);
  for (const [index, id] of ['malformed', 'wrong-kind', 'invalid-analysis'].entries()) {
    answer(db, id, NOW - index);
    observation(db, id, NOW - index - 1, { status: id === 'invalid-analysis' ? 'invalid' : 'ok' });
    receipt(db, id, { kind: id === 'wrong-kind' ? 'ignored' : 'answered' });
  }
  db.prepare('UPDATE runtime_inbound_update_receipts SET result_json = ? WHERE receipt_id = ?')
    .run('{malformed', 'malformed');
  db.close();
  const result = assistantCostAnalytics(config, { nowSeconds: NOW });
  assert.equal(result.status, 'available');
  assert.equal(result.total.pricedCount, 0);
  assert.equal(result.lastFive.unknownCount, 3);
  assert.equal(result.lastFive.cost, null);
  for (const row of result.lastFive.rows) assert.equal(row.stages.router.known, false);
});

test('measured zero and partial deliveries keep correct denominators without database writes', (t) => {
  const { db, config } = fixture(t);
  const deliveries = ['ok', 'partial', 'degraded:plain_text', 'failed'];
  for (const [index, delivery] of deliveries.entries()) {
    const id = `delivery-${index}`;
    const zero = index === 0;
    answer(db, id, NOW - index, { delivery, input: zero ? 0 : 1000,
      output: zero ? 0 : 100, total: zero ? 0 : 1100 });
    observation(db, id, NOW - index - 1, { input: zero ? 0 : 500,
      output: zero ? 0 : 50, total: zero ? 0 : 550 });
    receipt(db, id);
  }
  db.close();
  const hash = () => createHash('sha256').update(readFileSync(config.runtimeDatabasePath)).digest('hex');
  const before = hash();
  const result = assistantCostAnalytics(config, { nowSeconds: NOW });
  assert.equal(hash(), before, 'read model cannot alter its input database');
  assert.equal(result.lastFive.pricedCount, 3);
  assert.equal(result.lastFive.unknownCount, 1);
  assert.equal(result.lastFive.rows[0].estimatedUsd, 0);
  near(result.lastFive.rows[1].estimatedUsd, 0.00336);
  near(result.lastFive.rows[2].estimatedUsd, 0.00336);
  assert.equal(result.lastFive.rows[3].estimatedUsd, null);
  assert.equal(result.lastFive.rows[3].knownStagesUsd, null);
  assert.equal(result.lastFive.cost, null);
  near(result.lastFive.averageCostUsd, 2 * 0.00336 / 3);
});
