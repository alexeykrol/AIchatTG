...
- `target` — адресат обнаруженной угрозы или абьюза, а не адресат обычного
  вопроса. Если оба `match=false`, обязательно `target="none"` и
  `context_used=false`, даже когда вопрос обращён к ассистенту или не относится
  к теме курса. Если хотя бы один `match=true`, `target` не может быть `none`.
- `context_used=true` допустимо только когда `abuse.types` содержит
  `warning_dispute` и передан реальный контекст предыдущего предупреждения
  (`weak_strikes>0`, `warning_stage` не `none`). Для `warning_dispute` этот
  контекст обязателен. Во всех остальных случаях `context_used=false`.
