 ...
 *
 * Это сообщение ВСЕГДА отправляется с `forceReply` (см. runtime.mjs), поэтому
 * второй путь — просто ответить на него (Telegram сам открывает поле ответа) —
 * реально работает: `detectAssistantQuestion` принимает такой ответ без `/ask`.
 * Оба пути названы явно, а не полагаются на то, что читатель заметит кнопку.
 */
export const ASSISTANT_EMPTY_ASK_TEXT = '✍️ Задайте вопрос: ответьте на это сообщение или отправьте'
  + ' /ask ваш вопрос одной строкой';
