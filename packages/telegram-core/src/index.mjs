...
// Сравнение продукта с бесплатной альтернативой — «чем ваш курс отличается от
// роликов на ютубе». Вопрос о ПРОДУКТЕ, не о предмете, поэтому якорь — слово
// продукта (курс/обучение/программа) рядом со сравнением И маркер альтернативы.
// Без якоря шаблон ловил бы «чем поиск по документам отличается от дообучения»,
// то есть ровно содержательный вопрос.
// Найдено первым боевым прогоном журнала (2026-08-16, тестовый чат): модель
// уверенно ставила value, булев детектор молчал.
const VALUE_ALTERNATIVE = opsRx('(?:(?:курс|обучени|программ|тренинг|подписк)[^.!?]{0,30}(?:отлича|лучше|хуже|дешевле|дороже)[^.!?]{0,60}(?:бесплатн|ютуб|youtube|ролик|видео|блог|стать|книг|документаци|самостоятельно)|(?:бесплатн|ютуб|youtube|ролик)[^.!?]{0,60}(?:чем|вместо)[^.!?]{0,30}(?:курс|обучени|программ))');

// Наём ВМЕСТО освоения без слов «проще/дешевле», на которых стоял VALUE_PILL:
// «разве нельзя просто нанять человека и вообще не вникать». Якорь двойной —
// наём И явный отказ вникать, поэтому «кого нанять на позицию» сюда не попадает.
// Найдено тем же боевым прогоном.
const VALUE_HIRE_INSTEAD = opsRx('(?:(?:наня|нанима|найму|нанять)[^.!?]{0,60}не (?:вникать|разбираться|учиться|погружаться|лезть)|не (?:вникать|разбираться|учиться|погружаться)[^.!?]{0,60}(?:наня|нанима|найму|подрядчик|исполнител))');

// Обесценивание учёбы формулой «какой смысл это учить». Отличается от законного
// содержательного «зачем изучать X» тем, что спрашивается смысл САМОГО учения,
// без предмета: голый `зачем изучать` в шаблон не входит намеренно.
// Найдено тем же боевым прогоном.
const VALUE_WHY_LEARN = opsRx('(?:какой|каков|в чем)[^.!?]{0,20}смысл[^.!?]{0,25}(?:это |этому |их |все это )?(?:учить|учиться|изучать|осваивать|разбираться|проходить)');
...
export function isCourseValueQuestion(text) {
  ...
  return VALUE_SUITABILITY.test(normalized) || VALUE_BENEFIT.test(normalized)
    ...
    || VALUE_PILL_SHORTCUT.test(normalized) || VALUE_ALTERNATIVE.test(normalized)
    || VALUE_HIRE_INSTEAD.test(normalized) || VALUE_WHY_LEARN.test(normalized);
}
